#!/usr/bin/env node
/**
 * Static-site generator for the General Slider demo library.
 *
 * Reads general-slider/demo-library.json (+ each demos/{id}.json) — the single
 * source of truth — and generates:
 *   general-slider/index.html          → a browsable demo gallery
 *   general-slider/{id}/index.html     → a documentation page per demo (own URL)
 *   general-slider/site.css            → shared styles
 *
 * Re-run after adding or changing a demo:  node build-general-slider.js
 */
'use strict';
...
const ROOT = path.join(__dirname, 'general-slider');
...
// Merge each manifest entry with its full demo file (slides + settings).
const full = demos.map((d) => {
	...
	return { ...d, slides: detail.slides || [], settings: detail.settings || {} };
});
...
function buildIndex() {
	...
	const html = `${head(
		'General Slider — Demo Library',
		'Browse ready-made General Slider demos for WordPress: hero, split, testimonial, carousel, fullscreen and minimal sliders. Import any demo in one click.',
		'site.css',
		BASE
	)}
...
function buildDemo(d) {
	...
	const s = d.settings || {};
	const metaRows = [
		['Requires', d.requires ? `General Slider ${esc(d.requires)}+` : '—'],
		['Version', esc(d.version || '1.0')],
		['Updated', esc(d.updated || '—')],
		['Category', esc(d.category || '—')],
		['Preset', esc(s.preset || '—')],
		['Slides', esc((d.slides || []).length)],
	]
	...
	const slides = (d.slides || [])
		.map((sl, i) => {
			const img = sl.image_url ? '../' + rel(sl.image_url) : '';
			const thumb = img
				? `<div class="slide__thumb"><img src="${esc(img)}" alt="${esc(sl.alt || sl.heading || 'Slide ' + (i + 1))}" loading="lazy"></div>`
				: '';
			const btn = sl.btn_text ? `<span class="slide__btn">${esc(sl.btn_text)}</span>` : '';
			return `<li class="slide">
	${thumb}
	<div class="slide__text">
		${sl.sub_heading ? `<p class="slide__sub">${esc(sl.sub_heading)}</p>` : ''}
		${sl.heading ? `<h3 class="slide__title">${esc(sl.heading)}</h3>` : ''}
		${sl.text ? `<p class="slide__body">${esc(sl.text)}</p>` : ''}
		${btn}
	</div>
</li>`;
		})
...
<main class="wrap demo">
	...
	<section class="panel">
		<h2>How to import</h2>
		<ol class="steps">
			<li>Install and activate <a href="${PLUGIN_WPORG}">General Slider</a> on your WordPress site.</li>
			<li>Go to <strong>General Slider → Demo Library</strong> in wp-admin.</li>
			<li>Find <strong>${esc(d.name)}</strong> and click <strong>Import Demo</strong> — the slider and its images are added automatically.</li>
		</ol>
	</section>
	...
	<section class="panel">
		<h2>Slides in this demo</h2>
		<ul class="slides">
${slides}
		</ul>
	</section>

	<p class="links"><a class="btn" href="${PLUGIN_WPORG}">Get General Slider</a> <a class="btn btn--ghost" href="../">Browse more demos</a></p>
</main>
<footer class="site-footer"><div class="wrap"><p>General Slider demo library · <a href="${PLUGIN_WPORG}">WordPress.org</a> · <a href="${GITHUB}">GitHub</a></p></div></footer>
</body></html>`;
...
/* ---------- Shared CSS ---------- */
const CSS = `:root{--accent:#2563eb;--ink:#1f2937;--muted:#6b7280;--line:#e5e7eb;--bg:#fff;--soft:#f8fafc}
...
.slides{list-style:none;margin:0;padding:0;display:grid;gap:16px}
.slide{display:flex;gap:16px;align-items:flex-start;border:1px solid var(--line);border-radius:12px;padding:12px;background:var(--soft)}
.slide__thumb{flex:0 0 160px}
.slide__thumb img{width:160px;height:90px;object-fit:cover;border-radius:8px;display:block}
.slide__sub{margin:0 0 4px;text-transform:uppercase;letter-spacing:.08em;font-size:11px;font-weight:700;color:var(--accent)}
.slide__title{margin:0 0 4px;font-size:17px}
.slide__body{margin:0 0 8px;color:var(--muted);font-size:14px}
.slide__btn{display:inline-block;font-size:12px;font-weight:600;background:var(--ink);color:#fff;padding:5px 12px;border-radius:6px}
.site-footer{border-top:1px solid var(--line);padding:26px 0;color:var(--muted);font-size:14px}
.site-footer a{color:var(--muted)}
@media(max-width:560px){.slide{flex-direction:column}.slide__thumb{flex:none}.slide__thumb img{width:100%;height:auto}.site-header h1{font-size:27px}}
`;
...
full.forEach((d) => console.log(`  general-slider/${d.id}/`));
